...
data_pd_training = data_pd_training[0:20000]
...
# train model
model = GCNModel(mode='regression',model_dir = 'model',graph_conv_layers=[64,128],dense_layer_size=256,dropout=0.1,
	n_tasks=1,batch_size=128,learning_rate=0.001)
loss = model.fit(dataset, nb_epoch=500)
print(loss)
...
# restore the model
model = GCNModel(mode='regression',graph_conv_layers=[64,128],dense_layer_size=256,dropout=0.1,n_tasks=1,batch_size=128,learning_rate=0.001)
...
# # select the first 500 rows
# data_pd_validation = data_pd_validation[0:500]
# extract the smiles and labels
...
plt.scatter(prediction,data_pd_validation['logP'].to_numpy(),s=0.5)
